...
int main() {
	...
	while (T--) {
		...
		cin >> N >> M;
		MCMF mcmf(2 * N + 3);
		mcmf.add_edge(0, 2 * N + 1, 2, 0);
		while (M--) {
			int u{}, v{};
			cin >> u >> v;
			mcmf.add_edge(N + u, v, 1, 0);
		}
		for (int i = 1; i <= N; ++i) {
			mcmf.add_edge(2 * N + 1, i, INF, 0);
			mcmf.add_edge(i, N + i, 1, -1);
			mcmf.add_edge(N + i, 2 * N + 2, INF, 0);
		}
		cout << -mcmf.mcmf(0, 2 * N + 2).second << "\n";
	}
	return 0;
}
