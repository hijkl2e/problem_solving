...
int main() {
	...
	for (int i = 0; i < N; ++i) {
		for (int j = 0; j < N; ++j) {
			mcmf.add_edge((N + i) * N + j, i * N + j, 1, -A[i][j]);
			mcmf.add_edge((N + i) * N + j, i * N + j, K - 1, 0);
			if (i < N - 1) {
				mcmf.add_edge(i * N + j, (N + i + 1) * N + j, INF, 0);
			}
			if (j < N - 1) {
				mcmf.add_edge(i * N + j, (N + i) * N + j + 1, INF, 0);
			}
		}
	}
	cout << -mcmf.mcmf(N * N, N * N - 1).second << "\n";
	return 0;
}
