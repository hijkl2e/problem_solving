...
bool operator<(const line &l1, const line &l2) {
	...
	ll c = (l1.p2.y - l1.p1.y) * (l2.p2.x - l2.p1.x);
	ll d = (l2.p2.y - l2.p1.y) * (l1.p2.x - l1.p1.x);
	return ((l2.p2.x - l2.p1.x) * a < (l1.p2.x - l1.p1.x) * b)
			|| ((l2.p2.x - l2.p1.x) * a == (l1.p2.x - l1.p1.x) * b && c < d);
}
...
bool are_intersect(const line &l1, const line &l2) {
	...
	if ((r1 | r2 | r3 | r4) == 0) {
		return true;
	} else if (l1.p1 == l2.p1 || l1.p2 == l2.p2) {
		return false;
	} else {
...
int main() {
	...
	for (int i = 0; i < N; ++i) {
		...
		E.push_back({l.p1.x, 1, l.p1.y, i});
		E.push_back({l.p2.x, 0, l.p2.y, i});
	}
	sort(E.begin(), E.end());
	bool yes{};
	multiset<line> S;
